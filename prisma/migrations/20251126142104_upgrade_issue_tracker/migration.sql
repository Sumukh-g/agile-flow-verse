...
-- Step 4: Migrate existing data - only if table exists and columns are still TEXT type
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'issues') THEN
    -- Only migrate data if columns are still TEXT type (not already converted to enums)
    IF EXISTS (
