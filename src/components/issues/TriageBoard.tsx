...
import type { Issue, IssueStatus, IssuePriority, IssueType, IssueSeverity } from '@/lib/api/issues';
import { useUpdateIssueStatus, useUpdateIssue } from '@/hooks/useIssues';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';

const TRIAGE_COLUMNS: { status: IssueStatus; label: string; color: string }[] = [
  { status: 'INBOX', label: 'Inbox', color: 'bg-gray-100' },
  { status: 'NEEDS_INFO', label: 'Needs Info', color: 'bg-yellow-100' },
  { status: 'TRIAGED', label: 'Triaged', color: 'bg-blue-100' },
];
...
export const TriageBoard: React.FC<TriageBoardProps> = ({
  ...
  const handleQuickUpdate = async (issue: Issue, field: 'priority' | 'type' | 'severity', value: string) => {
    try {
        ...
        <div className="flex items-start justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => handleQuickUpdate(issue, 'type', 'BUG')}>
                Mark as Bug
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleQuickUpdate(issue, 'type', 'STORY')}>
                Mark as Story
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Triage Board</h2>
          <p className="text-sm text-muted-foreground">Review and categorize incoming issues</p>
        </div>
        {onCreateIssue && (
