...
interface UpdateTenantFeaturesRequest {
  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { error: 'Unauthorized - sys_admin role required' },
        { status: 403 }
      );
    }

    const tenantId = params.id;
    const body: UpdateTenantFeaturesRequest = await request.json();
    ...
    if (!body.features || !Array.isArray(body.features)) {
      return NextResponse.json(
        { error: 'Invalid request body - features array required' },
        { status: 400 }
      );
    }
    ...
    for (const feature of body.features) {
      if (!feature.key || typeof feature.enabled !== 'boolean') {
        return NextResponse.json(
          { error: 'Invalid feature update - key and enabled required' },
          { status: 400 }
        );
      }
    }
